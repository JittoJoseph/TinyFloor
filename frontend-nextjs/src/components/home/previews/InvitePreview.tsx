...
/** An invite link being handed out, and the teammate who used it, in on the floor. */
  ...
  return (
    <div className="font-(family-name:--font-app) [font-feature-settings:'cv11','ss01'] flex h-full flex-col gap-3 p-4 text-start">
      <div className="rounded-2xl border border-border bg-background p-3.5 shadow-[0_12px_32px_-18px_rgb(0_0_0/0.3)]">
        ...
        <p className="mt-0.5 text-[11.5px] text-muted-foreground">{t("inviteNote")}</p>
      ...
      <FloorScene
        ...
        over={
          <span className="absolute start-2.5 top-2.5 flex items-center gap-2 rounded-full border border-border bg-card py-1 pe-3 ps-1 shadow-float [--face-ring:var(--ui-card)]">
            <Face seed={grace.id} size={20} />
            <span className="text-[11.5px] text-foreground">
              <span className="font-semibold">{grace.name}</span> {t("joined")}
            </span>
          </span>
        }
      />
