...
/** The first part of every app route, after the locale. The landing pages are everything else. */
 ...
 * touches app routes; on a landing page it does nothing, and AppTheme takes
 * over on client-side navigation. Kept as a string because it runs before any
 * bundle has loaded.
 */
export const THEME_SCRIPT = `try{var p=location.pathname.split("/").filter(Boolean);if(p[0]&&p[0].length===2)p.shift();if(${JSON.stringify(
  APP_SECTIONS,
