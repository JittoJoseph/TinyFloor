      ...
      <div role="tablist" aria-label={label} className="grid grid-cols-4 gap-1 p-0.5">
        {VIEWS.map((one) => (
          <button
            ...
            className={cn(
              "relative h-9 cursor-pointer overflow-hidden rounded-full text-[13px] font-medium transition-colors sm:h-10 sm:text-[13.5px]",
              view === one
          ...
          >
            ...
            {view === one && !picked && (
              <span
                aria-hidden
                key={one}
                className="home-dwell absolute inset-x-5 bottom-1 h-[2px] origin-left rounded-full bg-foreground/15 motion-reduce:hidden"
