...
export function GuestPreview() {
  ...
  return (
    <div className="font-(family-name:--font-app) [font-feature-settings:'cv11','ss01'] flex h-full flex-col gap-3 p-4 text-start">
      ...
      <SceneMedia name="guest" className="min-h-0 flex-1 rounded-2xl border border-border" />
    </div>
  );
}

/** Grace walking onto the floor through the link, and the note that says she's in, on the same 12s loop. */
export function GuestFloor({ className }: { className?: string }) {
  const t = useTranslations("home.preview");
  const { grace } = PEOPLE;
  return (
    <>
