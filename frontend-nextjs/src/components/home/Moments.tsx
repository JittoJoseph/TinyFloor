 ...
 * up; on the right you, walked over to Jack's desk and already talking. Each side ends on how long it took.
 */
    ...
    <section id="floor" className={cn(COLUMN, "m-still scroll-mt-20 pt-24 sm:pt-36")}>
      ...
      <div className="mt-12 grid grid-cols-1 gap-3 lg:mt-14 lg:grid-cols-2">
        ...
        <div className="flex flex-col overflow-hidden rounded-[28px] border border-border bg-card p-6 sm:p-8">
          ...
          <WalkedOver className="mt-6 h-[260px] rounded-[20px] sm:h-[288px]" />
 ...
 */
/**
 * The floor side of the question: you, beside Jack's desk, and the call you
 * started there along the top, Jack talking.
 */
function WalkedOver({ className }: { className?: string }) {
  const t = useTranslations("home.versus");
  ...
  return (
    <FloorScene
      ...
      className={className}
      sitting={[
        { ...jack, chair: [34, 16], status: "in_call" },
        { ...PEOPLE.noah, chair: [34, 13], status: "busy" },
        { ...PEOPLE.sam, chair: [40, 13], status: "busy" },
        { ...PEOPLE.grace, chair: [40, 16], status: "away" },
      ]}
      standing={[
        { ...emma, name: t("after.you"), at: [32, 15], face: "right", status: "in_call" },
        { ...PEOPLE.olivia, at: [42, 9], face: "left", status: "available" },
      ]}
      over={
        <div className="pointer-events-none absolute inset-x-0 top-3 flex justify-center gap-2 px-3 [--face-ring:var(--ui-card)]">
          {[
            { seed: emma.id, name: t("after.you"), talking: false },
            { seed: jack.id, name: jack.name, talking: true },
          ].map((card) => (
            <div
              key={card.seed}
              className={cn(
                APP,
                "relative aspect-video w-[min(40%,10.5rem)] overflow-hidden rounded-xl bg-card shadow-lg ring-2",
                card.talking ? "ring-brand" : "ring-card/80",
              )}
            >
    ...
    },
    {
      ...
      art: (
        <FloorScene
          ...
          standing={[
            { ...PEOPLE.emma, at: [22, 14], face: "right", status: "available" },
