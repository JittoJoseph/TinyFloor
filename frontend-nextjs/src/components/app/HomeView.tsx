...
import { ArrowRight, ChevronDown, ChevronRight, Plus, Settings, UserPlus } from "lucide-react";
  ...
  return (
    <>
      <div className="rounded-2xl border border-dashed border-border-strong px-6 py-12 text-center">
        <span className="mx-auto flex size-11 items-center justify-center rounded-xl bg-muted text-foreground">
          <Plus className="size-5" />
        </span>
        <h2 className="mt-4 text-[17px] font-semibold tracking-tight text-foreground">{t("emptyTitle")}</h2>
        <p className="mx-auto mt-1.5 max-w-sm text-[13.5px] leading-relaxed text-muted-foreground">{t("emptyBody")}</p>
        <Link href="/create" className={cn(buttonClass.primary, "mt-5")}>
      ...
      <Link
        href={lobbyPath}
        className="mx-auto mt-6 flex w-fit items-center gap-1 text-[12.5px] text-muted-foreground transition-colors hover:text-foreground"
      >
        {t("lobbyNote")}
        <ChevronRight className="size-3.5 rtl:rotate-180" />
      </Link>
