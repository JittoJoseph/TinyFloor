...
const getPresence = (room: DirectoryRoom): Presence => {
...
/**
 * Starts from the rooms the server rendered, when it could reach the backend,
 * and refreshes them in place instead of showing a spinner.
 */
export function RoomsDirectory({
  initialRooms,
}: {
  initialRooms: DirectoryRoom[] | null;
}) {
  const t = useTranslations("directory");
  const tc = useTranslations("common");
  const timeAgo = useTimeAgo();
  const [rooms, setRooms] = useState<DirectoryRoom[]>(initialRooms ?? []);
  const [loading, setLoading] = useState(!initialRooms);
  const [loadingMore, setLoadingMore] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [hasMore, setHasMore] = useState(
    initialRooms ? initialRooms.length === ROOMS_PAGE_SIZE : true,
  );
  ...
  const fetchRooms = async (pageToLoad = 0, append = false) => {
    ...
    try {
      const data = await apiClient.getRooms(pageToLoad, ROOMS_PAGE_SIZE);
      const normalized = toDirectoryRooms(data);
      setRooms((prev) => (append ? [...prev, ...normalized] : normalized));
      pageRef.current = pageToLoad;
      setHasMore(normalized.length === ROOMS_PAGE_SIZE);
  ...
  useEffect(() => {
    const refresh = async () => {
      try {
        const fresh = toDirectoryRooms(
          await apiClient.getRooms(0, ROOMS_PAGE_SIZE),
        );
        setRooms(fresh);
        setHasMore(fresh.length === ROOMS_PAGE_SIZE);
      } catch (error) {
        console.error("Failed to fetch rooms:", error);
      } finally {
        setLoading(false);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setRooms(toDirectoryRooms(data));
      setIsSearching(true);
      setHasMore(false);
      pageRef.current = 0;
    } catch (error) {
                    ...
                    <div className="mt-4 flex-1">
                      ...
                      {room.lastActivityAt && (
                        <p
                          ...
                          className="text-[13px] text-[var(--color-braun-text)] opacity-60 mt-1"
                        >
                          {t("lastActive", {
